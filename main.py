...
class MainApplication:
    ...
    def create_cic_frame(self, frame, color_theme):
        ...
        # "Calculate" button
        calculate_button = tk.Button(frame, text = "Calculate", font = color_theme["font_style_entry"], bg = color_theme["btn_background_color"], fg = color_theme["text_color"], borderwidth = 0, highlightbackground = color_theme["highlight_background"], activebackground = color_theme["active_background"], command = lambda: MainApplication.calculate_ci(display_result, color_theme, principal, period, interest))
        calculate_button.grid(row = 4, columnspan = 2, pady = (30, 0))

        frame.pack(expand = False)

    # Function to calculate the compound interest
    def calculate_ci(textbox, color_theme, principal, period, interest):
        try:
            error_occured = False
            ...
            if not (MainApplication.is_float(p) and p):
            ...
            if not (MainApplication.is_integer(n) and n):
            ...
            if not (MainApplication.is_float(i) and i):
